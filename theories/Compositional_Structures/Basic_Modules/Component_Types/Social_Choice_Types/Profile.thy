...
lemma win_count_max:
  shows "win_count p x \<le> length p"
proof-
  have "{i::nat. i < length p \<and> above (p!i) x = {x}} \<subseteq> {0..<length p}"
    by auto
  hence "card {i::nat. i < length p \<and> above (p!i) x = {x}} \<le> card {0..<length p}"
...
lemma add_zero: "(n::nat) + 0 = n"

proof -
  have "(n + 0) = n" unfolding nat_def using nat_int by simp
  then show ?thesis
    by simp 
qed
